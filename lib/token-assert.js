...
TokenAssert.prototype.spacesBetween = function(options) {
    ...
    var emitError = function(countPrefix, spaceCount) {
        ...
        var msgPostfix = this._formatToken(token) + ' and ' + this._formatToken(nextToken, true);
...
/**
 * Formats token to be friendly in error messages.
 *
 * @param {Object} token
 * @param {Boolean} asNext
 * @returns {String}
 */
TokenAssert.prototype._formatToken = function(token, asNext) {
    if (!token.isComment) {
        return token.value;
    }
    ...
    if (token.type === 'Line') {
        val = _rtrim(val);
        return asNext ?
            '//' + _rtrim(val.substr(0, MAX_LENGTH)) + '...' :
            '//...' + _trim(val.substr(-(val.length - MAX_LENGTH - 1)));
    }

    val = '/*' + val.replace(/\r/, '') + '*/';
    var eol = val.indexOf('\n');
    if (eol === -1 && val.length < MAX_LENGTH + 3 /* dots */) {
        return val;
    }

    if (asNext) {
        return _trim(val.substr(0, eol === -1 ?
            MAX_LENGTH : Math.min(MAX_LENGTH, eol))) + '...';
    }

    return '...' + _trim(val.substr(-(eol === -1 ?
        MAX_LENGTH : Math.min(val.length - MAX_LENGTH - 1, val.lastIndexOf('\n')))));
};
...
TokenAssert.prototype.linesBetween = function(options) {
    ...
    var emitError = function(countPrefix, lineCount) {
        var msgPrefix = this._formatToken(token) + ' and ' + this._formatToken(nextToken, true);
...
TokenAssert.prototype.tokenBefore = function(options) {
    ...
    if (!actualTokenBefore) {
        this.emit('error', {
            message: this._formatToken(expectedTokenBefore) + ' was expected before ' +
                this._formatToken(token, true) + ' but document start found',
    ...
    if (
        ...
        if (!message) {
            ...
            message =
                this._formatToken(expectedTokenBefore) + (showTypes ? ' (' + expectedTokenBefore.type + ')' : '') +
                ' was expected before ' + this._formatToken(token, true) +
                ' but ' + this._formatToken(actualTokenBefore) +
                    (showTypes ? ' (' + actualTokenBefore.type + ')' : '') + ' found';
        }
...
TokenAssert.prototype.noTokenBefore = function(options) {
    ...
    if (actualTokenBefore.type === expectedTokenBefore.type &&
        ...
        this.emit('error', {
            message: options.message || 'Illegal ' + this._formatToken(expectedTokenBefore) + ' was found before ' +
                this._formatToken(token),
            line: actualTokenBefore.loc.end.line,
            column: actualTokenBefore.loc.end.column,
            fixed: fixed
        });
